...
public class IndexedDocument implements StatementAction {
    ...
    private final HashMap<QualifiedName, Collection<MentionOf>> namedMentionOfMap= new HashMap<>();
    private final HashMap<QualifiedName, Collection<MentionOf>> specificEntityMentionOfMap= new HashMap<>();
    private final Collection<MentionOf> anonMentionOf=new LinkedList<MentionOf>();
    private final HashMap<QualifiedName, Collection<MentionOf>> genericEntityMentionOfMap= new HashMap<>();
    ...
    public MentionOf add(MentionOf men) {
        return add(men, 2, anonMentionOf, namedMentionOfMap, specificEntityMentionOfMap, genericEntityMentionOfMap);
    }
    ...
    private void toContainer(List<StatementOrBundle> statementOrBundle) {
        ...
        statementOrBundle.addAll(anonMentionOf);
    ...
    //This should be used by prov-template compiler
    /*
    public Set<Pair<QualifiedName,MentionOf>> traverseMentionsWithRelations(QualifiedName from) {
        Stack<QualifiedName> s=new Stack<>();
        s.push(from);
        return traverseMentions2(new HashSet<>(),new HashSet<>(),s);
    }

    public Set<Pair<QualifiedName,MentionOf>> traverseMentions2(Set<Pair<QualifiedName,MentionOf>> last, Set<QualifiedName> seen, Stack<QualifiedName> todo) {
        ...
        while (!(todo.isEmpty())) {
            ...
            if (seen.contains(current)) {
                ...
                seen.add(current);
                Collection<MentionOf> successors=genericEntityMentionOfMap.get(current);
                if (successors==null || successors.isEmpty()) {
                    //move on next
                } else {
                    for (MentionOf men: successors) {
                        QualifiedName qn=men.getSpecificEntity();
                        last.add(Pair.of(qn,men));
                        // This makes not sense to have this: todo.push(qn);
                    }
                }
            }
